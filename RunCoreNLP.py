...
def coref_dir(params):
    ...
    dirname, data_dir, char_dir, output_dir, test_csv_dir = params
    ...
    if not char_dir.startswith('/'): # relative path
        command = "./run.sh " + data_dir +  " ../" + char_dir + " ../" + output_dir
        os.system(command)
    ...
    os.chdir("..")
    print("Processing the outputs..") # Turn text files to CSV
    #for filename in filenames:
    #for filename in os.listdir(split_data_dir):
    for filename in os.listdir(data_dir):
        
        fname = output_dir+"/"+filename+".coref.txt"
        exists = os.path.isfile(fname)

        if(exists):
            f = open(fname)
            #fin = open(test_csv_dir+filename, encoding='cp1250')
            fin = open(test_csv_dir+filename + ".csv")
            reader = csv.reader(fin)
            header = next(reader)
            #fout = open(output_dir+"/"+filename[:-4]+".coref.csv","w", encoding='cp1250')
            fout = open(output_dir+"/"+filename+".coref.csv","w", encoding='utf8')
...
def main():
    ...
    make_text_files = True # for debugging
    ...
    if n_threads > 1:
        with Pool(n_threads) as p:
            list(tqdm(p.imap(coref_dir, list(zip(
                ...
                itertools.repeat(test_csv_dir),
            ))), total=90, ncols=50))

    else:
        if n_dir_splits == 0:
            coref_dir((data_dir, data_dir, char_dir, output_dir, test_csv_dir))
    
        else:
            list(tqdm(map(coref_dir, list(zip(
                os.listdir(data_dir), 
                itertools.repeat(data_dir),
                itertools.repeat(char_dir),
                itertools.repeat(output_dir),
