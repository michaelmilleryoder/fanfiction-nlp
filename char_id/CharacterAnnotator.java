...
public class CharacterAnnotator {
	...
	public void findCharacters(Book book, Dictionaries dicts) {
		...
		System.out.println("Subsets of names.");
		for (String name : characterHash) {


			HashSet<String> aliases1 = getAliases(name);
			...
			for (String name2 : characterHash) {
				...
				// if one characters's aliases contains
				// character's aliases then they are both same
				
				HashSet<String> aliases2 = getAliases("chandler");
				...
				if (!name.equals(name2) && aliases2.containsAll(nameSet)) {
					flag = true;
					continue;
				}
		...
		//Filter names using subset information from the wikia
		
		

		System.out.println("End of Subsets of names.");
	...
	/* return the aliases of main characters using wikia information */
		public HashSet<String> getAliases(String name) {
			
			HashSet<String> RachelAliases = Sets.newHashSet();
			HashSet<String> ChandlerAliases = Sets.newHashSet();
			HashSet<String> MonicaAiases = Sets.newHashSet();
			HashSet<String> JackAliases = Sets.newHashSet();
			HashSet<String> EmptyAliases = Sets.newHashSet();
			EmptyAliases.add(name);
			
			RachelAliases.add("Rach");
			RachelAliases.add("Green");
			RachelAliases.add("Ray-Ray");
			RachelAliases.add("Raquel");
			RachelAliases.add("Raquela");
			RachelAliases.add("Mrs. Geller");
			RachelAliases.add("Dear");
			RachelAliases.add("Honey");
			
			
			
			ChandlerAliases.add("Ringo");
			ChandlerAliases.add("Mr. Big");
			ChandlerAliases.add("Sir-Limps-a-Lot");
			ChandlerAliases.add("Roland Chang");
			ChandlerAliases.add("Shaw-La-La");
			ChandlerAliases.add("Bing-a-Ling");
			ChandlerAliases.add("Miss Chanandler Bong");
			ChandlerAliases.add("Bing");
			ChandlerAliases.add("Chan-Chan Man");
			ChandlerAliases.add("The Dropper");
			ChandlerAliases.add("Mondler");
			ChandlerAliases.add("Clint");
			ChandlerAliases.add("Sir-Limps-a-Lot");
			ChandlerAliases.add("Roland Chang");
			ChandlerAliases.add("Channy");
			ChandlerAliases.add("Bing-a-Ling");
			ChandlerAliases.add("Mark Johnson");
			ChandlerAliases.add("John Markson");
			ChandlerAliases.add("Chan-Chan Man");
			ChandlerAliases.add("The Dropper");
			
				
			
			if (name.contains("rachel"))
			{
				return RachelAliases;
			}
			if (name.contains("chandler"))
			{
				return ChandlerAliases;
			}
			if (name.contains("monica"))
			{
				return MonicaAiases;
			}
