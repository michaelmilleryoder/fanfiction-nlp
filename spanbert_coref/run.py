...
class Runner:
    ...
    def evaluate(self, model, tensor_examples, stored_info, step, official=False, conll_path=None, tb_writer=None):
        ...
        doc_wise_scores_file = open("./data/doc_wise_scores_file.txt", "w")

        model.eval()
        for i, (doc_key, tensor_example) in enumerate(tensor_examples):
            doc_evaluator = CorefEvaluator()
            ...
            dummy = model.update_evaluator(span_starts, span_ends, antecedent_idx, antecedent_scores, gold_clusters, doc_evaluator)
            
            doc_p, doc_r, doc_f = doc_evaluator.get_prf()

            doc_wise_scores_file.write("=========================" + "\n")

            doc_wise_scores_file.write(doc_key + "\n")

            doc_wise_scores_file.write("CoNLL Avg" + "\n")
            doc_wise_scores_file.write(doc_p, doc_r, doc_f, "\n")

            doc_lp, doc_lr, doc_lf = evaluator.get_lea()

            doc_wise_scores_file.write("LEA" + "\n")
            doc_wise_scores_file.write(doc_lp, doc_lr, doc_lf, "\n")
    ...
    def save_model_checkpoint(self, model, step, total_step):
        if (1.0*step/total_step) < 0.5:
            return  # Debug
...
if __name__ == '__main__':
    config_name, gpu_id, suffix_name = sys.argv[1], int(sys.argv[2]), None
    if len(sys.argv) == 4:
        suffix_name = sys.argv[3]
    runner = Runner(config_name, gpu_id)
    model = runner.initialize_model(suffix_name)
